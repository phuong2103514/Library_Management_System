        ...
        <div class="container-fluid">
          ...
          <div class="page-header mb-4">
            <h2 class="fw-bold text-dark mb-2" style="font-size: 2.5rem">
              <i class="bi bi-person-lines-fill me-2"></i>Thống kê chi tiết
              phiếu đặt phòng theo độc giả
            </h2>
            <p>
              Quản lý và theo dõi chi tiết từng phiếu đặt phòng theo độc giả
              trong hệ thống
            </p>
          </div>
          ...
          <div class="card shadow-sm mb-4">
            <div class="card-body">
              <div class="row g-3">
                ...
                <div class="col-md-3">
                  <label class="form-label fw-semibold">Trạng thái đặt</label>
                  <select
                  ...
                  >
                    ...
                    <option value="waiting_members">Chờ thành viên</option>
                ...
                <div class="col-md-3">
                  ...
                  <input
                    ...
                    class="form-control"
                    placeholder="Tên phòng, mã phòng, độc giả..."
                    @input="applyFilters"
                    style="font-size: 1.3rem"
                  />
                </div>
            ...
            <div class="col-xl-3 col-md-6">
              ...
              >
                <div class="card-body text-white">
                  ...
                  >
                    <div>
                      <p class="mb-1 text-white-50">Tổng phiếu đặt</p>
                ...
                <table class="data-table">
                  ...
                  <tbody>
                    <tr v-for="(item, index) in paginatedData" :key="item._id">
                      ...
                      <td>
                        <div>
                          <div>{{ item.PhongHoc?.TenPhong || 'N/A' }}</div>
                          <div style="font-size: 1.2rem; color: #6c757d">
                            {{ item.PhongHoc?.MaPhong }}
                          </div>
                        </div>
                      </td>
                      <td>{{ item.PhongHoc?.LoaiPhong || 'N/A' }}</td>
                      <td>{{ formatDate(item.NgayDat) }}</td>
                      <td>{{ formatDate(item.NgaySuDung) }}</td>
                      <td style="text-align: center">{{ item.GioBatDau }}</td>
                      <td style="text-align: center">{{ item.GioKetThuc }}</td>
                      <td>
                        <span>
                      ...
                      <td style="text-align: center">
                        {{
                          item.ChoNgoiDaChon?.length > 0
                            ? item.ChoNgoiDaChon.join(', ')
                            : '-'
                        }}
                      </td>
                    </tr>
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" style="font-size: 2rem">
            <i class="bi bi-download me-2"></i>Xuất báo cáo phiếu đặt phòng
          </h5>
          <button
            type="button"
            class="btn-close"
...
export default {
  name: 'StatisticsBookingByReader',
  ...
  methods: {
    ...
    applyFilters() {
      ...
      if (this.searchKeyword) {
        ...
        data = data.filter((item) => {
          ...
          const roomCode = (item.PhongHoc?.MaPhong || '').toLowerCase();
          const readerName = `${item.DocGia?.HoLot || ''} ${
            item.DocGia?.Ten || ''
          }`.toLowerCase();
          const readerCode = (item.DocGia?.MaDocGia || '').toLowerCase();
    ...
    exportPDF() {
      ...
      const content = [
        {
          text: 'BÁO CÁO CHI TIẾT PHIẾU ĐẶT PHÒNG THEO ĐỘC GIẢ',
          style: 'header',
          alignment: 'center',
          margin: [0, 0, 0, 10],
        },
        ...
        {
          text: `Tổng số: ${exportData.length} phiếu đặt phòng`,
          style: 'summary',
          margin: [0, 0, 0, 20],
        },
      ];
      ...
      const tableBody = [
        [
          ...
          { text: 'Ngày sử dụng', style: 'tableHeader' },
          { text: 'Giờ bắt đầu', style: 'tableHeader' },
          { text: 'Giờ kết thúc', style: 'tableHeader' },
          { text: 'Trạng thái', style: 'tableHeader' },
          { text: 'Số TV', style: 'tableHeader' },
        ],
      ];
      ...
      exportData.forEach((item, index) => {
        tableBody.push([
          ...
          {
            text: item._id.substring(item._id.length - 8).toUpperCase(),
            style: 'tableCell',
          },
          {
            text: `${item.DocGia?.HoLot || ''} ${item.DocGia?.Ten || ''}\n${
              item.DocGia?.MaDocGia || ''
            }`,
            style: 'tableCell',
          },
          {
            text: `${item.PhongHoc?.TenPhong || 'N/A'}\n${
              item.PhongHoc?.MaPhong || ''
            }`,
            style: 'tableCell',
          },
          { text: item.PhongHoc?.LoaiPhong || 'N/A', style: 'tableCell' },
          { text: this.formatDate(item.NgayDat), style: 'tableCell' },
          { text: this.formatDate(item.NgaySuDung), style: 'tableCell' },
          { text: item.GioBatDau, style: 'tableCell', alignment: 'center' },
          { text: item.GioKetThuc, style: 'tableCell', alignment: 'center' },
          { text: this.getStatusText(item.TrangThai), style: 'tableCell' },
          {
            text: (item.PhongHoc?.LoaiPhong === 'Nhóm'
              ? item.ThanhVien?.length || 0
              : '-'
            ).toString(),
            style: 'tableCell',
      ...
      pdfMake
        ...
        .download(`bao-cao-dat-phong-theo-doc-gia-${new Date().getTime()}.pdf`);
    },
    ...
    exportExcel() {
      ...
      const excelData = [
        ['BÁO CÁO CHI TIẾT PHIẾU ĐẶT PHÒNG THEO ĐỘC GIẢ'],
        [],
      ...
      exportData.forEach((item, index) => {
        excelData.push([
          ...
          item.DocGia?.MaDocGia || '',
          `${item.DocGia?.HoLot || ''} ${item.DocGia?.Ten || ''}`,
          item.PhongHoc?.MaPhong || 'N/A',
          item.PhongHoc?.TenPhong || 'N/A',
          item.PhongHoc?.LoaiPhong || 'N/A',
          this.formatDate(item.NgayDat),
          this.formatDate(item.NgaySuDung),
          item.GioBatDau,
          item.GioKetThuc,
          ...
          item.PhongHoc?.LoaiPhong === 'Nhóm'
            ? item.ThanhVien?.length || 0
            : '-',
          item.ChoNgoiDaChon?.length > 0 ? item.ChoNgoiDaChon.join(', ') : '-',
        ]);
      });
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Phiếu đặt theo độc giả');
      XLSX.writeFile(
        wb,
        `bao-cao-dat-phong-theo-doc-gia-${new Date().getTime()}.xlsx`
      );
    },
  },
};
