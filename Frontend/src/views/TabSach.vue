          ...
          <div class="book__library-filter-content">
            <div class="form-check" v-for="r in ratingOptions" :key="r.value">
              <input
                ...
                type="checkbox"
                :value="r.value"
                :id="'sach-rating' + r.value"
                v-model="localSelectedRatings"
              />
              <label class="form-check-label" :for="'sach-rating' + r.value">
                <div class="rating">
...
export default {
  ...
  computed: {
    localSelectedGenres: {
      ...
      set(val) { this.$emit('update:selectedGenres', val); },
    },
    ...
    activeFilterCount() {
      const genres = (this.selectedGenres || []).length;
      ...
      return genres + ratings + publishers + years + keyword + author;
    },
  },
  watch: {
    // Khi chọn filter xong, tự đóng drawer trên mobile (tuỳ chọn — xoá nếu không muốn)
    localSelectedGenres() {
